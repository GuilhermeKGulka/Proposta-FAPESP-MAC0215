...
OUTPUT_CSV="results_cg.csv"

# Diretorio contendo as subpastas dos grupos/matrizes
MM_DIR="MM"

# 1. Verifica se o executavel do benchmark existe
if [ ! -f "./benchmark_cg" ]; then
    echo "Erro: Executavel './benchmark_cg' nao encontrado."
    echo "Compile primeiro com:"
    echo "  hipcc -O3 -c scalar_kernels.cpp -o scalar_kernels.o"
    echo "  hipfc -O3 mmio.f90 rocm_context.f90 benchmark_cg.f90 scalar_kernels.o -lhipsparse -lhipblas -o benchmark_cg"
    exit 1
fi
...
if [ ! -f "$OUTPUT_CSV" ] || [ ! -s "$OUTPUT_CSV" ]; then
    echo "Problem,N,NNZ,Density,CG_Iter_GPU,CG_Iter_CPU,Istop_GPU,Istop_CPU,Iter_Diff_Rel,t_GPU_copy_ms,t_GPU_CG_ms,t_CPU_CG_ms,Speedup,Speedup_with_copy" > "$OUTPUT_CSV"
    count=0
else
...
find -L "$MM_DIR" -type f -name "*.mtx" | sort | while read -r matrix_file; do
    ...
    if [[ "$filename" =~ _(b|x|z|c|coord)$ ]]; then
        continue
    fi
